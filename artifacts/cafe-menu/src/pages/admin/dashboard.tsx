...
function ImageUploadField({ value, onChange }: { value: string; onChange: (url: string) => void }) {
  ...
  const { uploadFile, isUploading, progress } = useUpload({
    onSuccess: (res) => { onChange(res.objectPath); setPreview(`/api/storage${res.objectPath}`); }
  });
  ...
  const handleClear = () => { onChange(""); setPreview(null); if (fileInputRef.current) fileInputRef.current.value = ""; };
    ...
    <div className="space-y-2">
      ...
      {preview ? (
        <div className="relative w-full h-36 rounded-lg overflow-hidden border border-border bg-muted">
          ...
          {!isUploading && (
            <button type="button" onClick={handleClear} className="absolute top-2 right-2 rounded-full bg-black/60 p-1 text-white hover:bg-black/80">
              <X className="h-3 w-3" />
            </button>
          )}
        </div>
...
export default function AdminDashboard() {
  ...
  const itemForm = useForm<MenuItemFormValues>({
    ...
    defaultValues: { name: "", description: "", price: 0, imageUrl: "", categoryId: 0, available: true, sortOrder: 0, isVeg: true, isBestseller: false, isSpicy: false },
  });
  ...
  if (meLoading || (activeTab === "overview" && (itemsLoading || summaryLoading))) {
  ...
  const handleLogout = () => {
    logoutMutation.mutate(undefined, {
      onSuccess: () => { queryClient.setQueryData(getGetMeQueryKey(), null); setLocation("/admin/login"); }
    });
  };
  ...
  const openEditItemDialog = (item: any) => {
    ...
    itemForm.reset({
      ...
      imageUrl: item.imageUrl || "", categoryId: item.categoryId, available: item.available,
      sortOrder: item.sortOrder, isVeg: item.isVeg ?? true,
      isBestseller: item.isBestseller ?? false, isSpicy: item.isSpicy ?? false,
    });
  ...
  const openNewItemDialog = () => {
    setEditingItem(null);
    itemForm.reset({ name: "", description: "", price: 0, imageUrl: "", categoryId: categories?.[0]?.id || 0, available: true, sortOrder: 0, isVeg: true, isBestseller: false, isSpicy: false });
    setItemDialogOpen(true);
  };

  const onItemSubmit = (data: MenuItemFormValues) => {
    const payload = { ...data, imageUrl: data.imageUrl || undefined, description: data.description || undefined };
    if (editingItem) {
      ...
      <header className="border-b border-border bg-card sticky top-0 z-20">
        <div className="max-w-7xl mx-auto px-4 h-14 flex items-center justify-between">
          <div className="flex items-center gap-6">
            <h1 className="text-lg font-serif font-semibold text-foreground">The Golden Brew</h1>
        ...
        {activeTab === "overview" && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <Card><CardHeader className="pb-2"><CardDescription>Total Items</CardDescription><CardTitle className="text-3xl">{summary?.totalItems ?? 0}</CardTitle></CardHeader></Card>
              <Card><CardHeader className="pb-2"><CardDescription>Categories</CardDescription><CardTitle className="text-3xl">{summary?.totalCategories ?? 0}</CardTitle></CardHeader></Card>
              <Card><CardHeader className="pb-2"><CardDescription>Pending Orders</CardDescription><CardTitle className="text-3xl text-yellow-500">{pendingOrders}</CardTitle></CardHeader></Card>
              <Card><CardHeader className="pb-2"><CardDescription>Today's Revenue</CardDescription><CardTitle className="text-3xl text-primary">₹{Math.round(todayRevenue)}</CardTitle></CardHeader></Card>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-card rounded-xl border border-border p-5">
                <h3 className="font-semibold text-foreground mb-3">Quick Links</h3>
                <div className="space-y-2">
                  <button onClick={() => setActiveTab("menu")} className="w-full text-left px-3 py-2.5 rounded-lg hover:bg-muted transition-colors text-sm text-muted-foreground hover:text-foreground flex items-center gap-2">
                    <UtensilsCrossed size={14} /> Manage menu items
                  </button>
                  <button onClick={() => setActiveTab("orders")} className="w-full text-left px-3 py-2.5 rounded-lg hover:bg-muted transition-colors text-sm text-muted-foreground hover:text-foreground flex items-center gap-2">
                    <ClipboardList size={14} /> View orders {pendingOrders > 0 && <Badge variant="destructive" className="text-[10px] px-1.5 py-0">{pendingOrders} pending</Badge>}
                  </button>
                  <button onClick={() => setActiveTab("qr")} className="w-full text-left px-3 py-2.5 rounded-lg hover:bg-muted transition-colors text-sm text-muted-foreground hover:text-foreground flex items-center gap-2">
                    <QrCode size={14} /> Generate QR codes
                  </button>
                  <button onClick={() => setActiveTab("settings")} className="w-full text-left px-3 py-2.5 rounded-lg hover:bg-muted transition-colors text-sm text-muted-foreground hover:text-foreground flex items-center gap-2">
                  ...
                  <div className="space-y-2">
                    {(orders as any[]).slice(0, 4).map((o: any) => (
                      <div key={o.id} className="flex items-center justify-between text-sm">
                        <span className="text-muted-foreground">Order #{o.id}{o.tableNumber ? ` · T${o.tableNumber}` : ""}</span>
                        <div className="flex items-center gap-2">
                          <span className="font-medium text-foreground">₹{Math.round(o.totalAmount)}</span>
                          <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-semibold ${
                            o.status === "pending" ? "bg-yellow-500/15 text-yellow-400" :
                            o.status === "completed" ? "bg-zinc-500/15 text-zinc-400" :
            ...
            <div className="w-full md:w-3/4 space-y-4">
                    ...
                    <Form {...itemForm}>
                      <form onSubmit={itemForm.handleSubmit(onItemSubmit)} className="space-y-4">
                        ...
                        <div className="grid grid-cols-3 gap-3">
                          ...
                          <FormField control={itemForm.control} name="isBestseller" render={({ field }) => (
                            <FormItem className="flex flex-row items-center justify-between rounded-lg border p-3">
                              <FormLabel className="text-sm">Best</FormLabel>
                              <FormControl><Switch checked={field.value} onCheckedChange={field.onChange} /></FormControl>
                            </FormItem>
                          )} />
                          <FormField control={itemForm.control} name="isSpicy" render={({ field }) => (
                            <FormItem className="flex flex-row items-center justify-between rounded-lg border p-3">
              ...
              <div className="bg-card rounded-xl border border-border overflow-hidden">
                <Table>
                  ...
                  <TableBody>
                    {items?.length === 0 ? (
                      <TableRow><TableCell colSpan={7} className="h-28 text-center text-muted-foreground">No items. Add your first item.</TableCell></TableRow>
                    ) : (
                      items?.sort((a, b) => a.categoryId - b.categoryId || a.sortOrder - b.sortOrder).map((item) => {
                        ...
                        return (
                          <TableRow key={item.id}>
                            <TableCell>
                              {imgSrc
                                ? <div className="w-9 h-9 rounded overflow-hidden bg-muted"><img src={imgSrc} alt={item.name} className="w-full h-full object-cover" /></div>
                                : <div className="w-9 h-9 rounded bg-muted flex items-center justify-center text-muted-foreground"><ImageIcon className="h-4 w-4" /></div>}
                            </TableCell>
                            <TableCell className="font-medium text-sm">{item.name}</TableCell>
                            <TableCell className="hidden md:table-cell text-sm text-muted-foreground">{item.categoryName}</TableCell>
                            <TableCell className="text-sm font-semibold">₹{Number(item.price).toFixed(0)}</TableCell>
                            <TableCell className="hidden sm:table-cell">
                              <div className="flex gap-1">
                                <span className={`w-3 h-3 rounded-sm border-2 flex items-center justify-center ${(item as any).isVeg ? "border-green-500" : "border-red-500"}`}>
                                  <span className={`w-1.5 h-1.5 rounded-full ${(item as any).isVeg ? "bg-green-500" : "bg-red-500"}`} />
                                </span>
                                {(item as any).isBestseller && <span className="text-[10px] px-1 bg-amber-500/15 text-amber-400 rounded font-semibold">⭐</span>}
                                {(item as any).isSpicy && <span className="text-[10px] px-1 bg-red-500/15 text-red-400 rounded font-semibold">🌶</span>}
                              </div>
                            </TableCell>
                            <TableCell className="hidden sm:table-cell">
                              {item.available
                                ? <Badge variant="outline" className="bg-green-500/10 text-green-500 border-green-500/20 text-xs"><CheckCircle className="w-3 h-3 mr-1" />On</Badge>
                                : <Badge variant="outline" className="bg-destructive/10 text-destructive border-destructive/20 text-xs"><XCircle className="w-3 h-3 mr-1" />Off</Badge>}
                            </TableCell>
                            <TableCell className="text-right">
                              <div className="flex items-center justify-end gap-1">
                                <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => openEditItemDialog(item)}><Edit2 className="h-3.5 w-3.5" /></Button>
                                <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive hover:text-destructive hover:bg-destructive/10" onClick={() => handleDeleteItem(item.id)} disabled={deleteItemMutation.isPending && (deleteItemMutation.variables as any)?.id === item.id}>
                    ...
                    <Form {...categoryForm}>
                      <form onSubmit={categoryForm.handleSubmit(onCategorySubmit)} className="space-y-4">
                        ...
                        <Button type="submit" className="w-full" disabled={createCategoryMutation.isPending}>
                          {createCategoryMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : null}Create
                        </Button>
                      </form>
                    </Form>
                  </DialogContent>
              ...
              <div className="bg-card rounded-xl border border-border overflow-hidden">
                <Table>
                  <TableBody>
                      ...
                      <TableRow key={c.id}>
                        ...
                        <TableCell className="text-right">
                          <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive hover:text-destructive hover:bg-destructive/10"
                            onClick={() => handleDeleteCategory(c.id)} disabled={deleteCategoryMutation.isPending && (deleteCategoryMutation.variables as any)?.id === c.id}>
                            <Trash2 className="h-3.5 w-3.5" />
                          </Button>
                        </TableCell>
                      </TableRow>
                    ...
                    {categories?.length === 0 && (
                      <TableRow><TableCell colSpan={2} className="text-center text-muted-foreground py-4 text-sm">No categories</TableCell></TableRow>
                    )}
                  </TableBody>
                </Table>
              </div>
